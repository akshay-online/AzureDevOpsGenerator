        ...
        public string GetDashBoardId(string projectName, string teamName = null)
        {
            try
            {
                ...
                using (var client = GetHttpClient())
                {
                    ...
                    {
                        DashboardResponse.Dashboard dashBoard = response.Content.ReadFromJsonAsync<DashboardResponse.Dashboard>().Result;
                ...
                using (var clientParent = GetHttpClient())
                {
                    ...
                    if (ResponseParent.IsSuccessStatusCode && ResponseParent.StatusCode == System.Net.HttpStatusCode.OK)
                    {
                        using (var client = GetHttpClient())
                        {
                            ...
                            if (response.IsSuccessStatusCode)
                            {
                                result = response.Content.ReadFromJsonAsync<QueryResponse>().Result;
                                return result;
                            }
                            else
                            {
        ...
        public QueryResponse GetQueryByPathAndName(string project, string queryName, string path)
        {
            try
            {
                using (var client = GetHttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        QueryResponse query = response.Content.ReadFromJsonAsync<QueryResponse>().Result;
                        return query;
                    }
                }
            }
            ...
            try
            {
                if (dashBoardId != "")
                {
                    using (var client = GetHttpClient())
                    {
                        ...
                        {
                            dynamic responseForInvalidStatusCode = response.Content.ReadFromJsonAsync<dynamic>();
